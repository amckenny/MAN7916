...
def read_dictionary(filename: str) -> dict[str, list[Any]]:
    ...
    if i < 4:
        raise ValueError(
            "CAT Scanner 1.0 .dict file is malformed (missing one or more sections). Cannot import."
        )
    ...
    if (
        ...
        raise ValueError(
            f"CAT Scanner 1.0 .dict file is malformed: {error_msgs}\nImport failed."
        )
